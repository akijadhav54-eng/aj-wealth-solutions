  ...
  return (
    <section id="blog" className="section">
      <Reveal className="text-center max-w-2xl mx-auto">
        ...
        <p className="text-ink/65 mt-4">Simple, practical money lessons to help you invest with confidence.</p>
      </Reveal>
          ...
          <Reveal key={post.slug} delay={i * 0.1}>
            <Link to={`/blog/${post.slug}`} className="group block h-full">
              <article className="h-full rounded-2xl bg-white ring-1 ring-ink/10 p-6 hover:ring-gold/40 hover:-translate-y-1 transition-all">
                ...
                <h3 className="font-display text-lg text-ink leading-snug group-hover:text-gold-dark transition-colors">
                  {post.title}
                </h3>
                <div className="flex items-center gap-4 text-ink/40 text-xs mt-4">
