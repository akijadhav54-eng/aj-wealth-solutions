      ...
      <div className="grid lg:grid-cols-2 gap-12 lg:gap-16 items-center">
        ...
        <Reveal>
          <div className="relative">
            ...
            <div className="relative rounded-3xl bg-white ring-1 ring-ink/10 p-8 sm:p-10">
              <img src="/logo.jpg" alt="AJ Wealth Solutions" className="h-20 w-20 rounded-full object-cover ring-2 ring-gold/40 mb-6" />
              <h3 className="font-display text-2xl text-ink">{site.owner}</h3>
              <p className="text-gold-dark text-sm mt-1">{site.role}</p>
              ...
              <div className="grid grid-cols-3 gap-4 mt-8">
                {stats.map((s) => (
                  <div key={s.label} className="text-center">
                    <s.icon className="mx-auto text-gold-dark mb-2" size={22} />
                    <div className="font-display text-lg text-ink">{s.value}</div>
                    <div className="text-[11px] text-ink/50 leading-tight mt-1">{s.label}</div>
                  </div>
                ))}
              </div>
            </div>
